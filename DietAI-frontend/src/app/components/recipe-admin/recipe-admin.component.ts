...
export class RecipeAdminComponent {
    ...
    public recipeWithIngredientsRequest: RecipeWithIngredientsRequest =
        new RecipeWithIngredientsRequest();
    ...
    registrarRecipe(recipe: Recipe) {
        if (this.ingredientsInRecipe.length == 0) {
            this.errorMessage = 'Debe registrar al menos un ingrediente';
            return;
        }
        this.recipeWithIngredientsRequest.recipe = recipe;
        this.recipeWithIngredientsRequest.ingredientInRecipe =
            this.ingredientsInRecipe;
        this.recipeService
            .postRecipe(this.recipeWithIngredientsRequest)
            .then((newRecipe) => {
    ...
    deleteRecipe() {
        ...
        this.recipeService
            .deleteRecipe(this.selectedRecipeId)
            .then((recipe) => {
